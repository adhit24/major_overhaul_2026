...
create table if not exists peserta (
  ...
  nama text not null,
  job_no text,
  departemen text check (departemen in ('ONE PLANT', 'INDIRECT', 'TBN-BOP', 'BOILER', 'SUPPORTING')),
...
-- No_Badge sengaja TIDAK unique: data riil punya badge yang terpakai 2x
-- secara historis (temuan rekonsiliasi). Cek duplikat untuk entri baru
-- dilakukan di app (lihat app/(app)/peserta/actions.ts), bukan di DB.
...
create table if not exists deposit_batch (
  ...
  departemen_section text check (departemen_section in ('ONE PLANT', 'INDIRECT', 'TBN-BOP', 'BOILER', 'SUPPORTING')),
  keterangan text,
  rentang_no_id text,
